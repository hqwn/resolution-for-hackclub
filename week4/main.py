...
def private(name: str):
    with sqlite3.connect("quotes.db") as background_conn:
            ...
            background_cursor.execute(
                "UPDATE quotes SET owner = 'Anonymous' WHERE owner = ?", 
                (name,)
            )
...
@app.patch("/quotes/incognito", dependencies=[Depends(verify_api_key)])
async def incognito(background_tasks: BackgroundTasks, key_info = Depends(verify_api_key)):
    background_tasks.add_task(private, key_info[2])
    return {"message": "Your quotes are now private!"}
